...
void Init_bodys2(body* bd, int N, double *center);
...
void Init_bd(body *bd, int N, double *min, double *max) {
    ...
    bd->a = (double *)malloc(3 * N * sizeof(double));

    if (bd->r == NULL || bd->v == NULL || bd->a == NULL ||bd->m == NULL ) {
        fprintf(stderr, "Error Allocation Memory Init_bd\n");
        // Aquí deberías liberar la memoria asignada previamente antes del error
        exit(EXIT_FAILURE);
    }

    for (size_t ii = 0; ii < N; ii++) {
        // random pos in domain
        bd->r[3*ii]     = randomDouble(min[0] + 20, max[0] - 20);  // x component
        bd->r[3*ii + 1] = randomDouble(min[1] + 20, max[1] - 20);  // y component
        bd->r[3*ii + 2] = randomDouble(min[2] + 20, max[2] - 20);  // z component
...
void Init_bd2(body* bd, int N, double *center) {
    ...
    bd->a = (double *)malloc(N * sizeof(double));
    ...
    for (size_t ii = 0; ii < N; ii++) {
        ...
        bd->r[3*ii ]    = center[0] + radius * cos(theta);
        bd->r[3*ii + 1] = center[1] + radius * sin(theta);
        bd->r[3*ii + 2] = center[2] + height;
...
int OctantIndex(Node *node, body *bd) {
    ...
    center[0] = (node->bbox[0][0] + node->bbox[1][0]) / 2;
    center[1] = (node->bbox[0][1] + node->bbox[1][1]) / 2;
    center[2] = (node->bbox[0][2] + node->bbox[1][2]) / 2;
