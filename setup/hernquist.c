...
void hernquist_dist(double *Pos, double *Vel, double *Mass, const int N) {
    double X1;
    double sqrt_cum_mass, r, Ve;
    double m = 1.0/N;
    ...
    for (ii = 0; ii < N; ii++) {
        Mass[ii] = m;
        // Position
        sqrt_cum_mass = sqrt(rndm(cum_mass_min, cum_mass_max));
        cum_mass_min = cum_mass_max;
        cum_mass_max += m;

        r = sqrt_cum_mass / (1 - sqrt_cum_mass); 
